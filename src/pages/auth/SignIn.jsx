...
const schema = yup.object({
    ...
    password_confirmation: yup.string().oneOf([yup.ref('password'),null],'Passsword does not match'),
  }).required();
...
const SignIn = () => {
    ...
    const onSubmit = async (data) => {
       ...
       if(res.error){
        const {data:{message}} = res.error
            setError(message)
       }
    }
  return (
    <div>
      <h1 className='text-purple-600 mb-3 text-2xl uppercase text-center font-bold'>Sign In</h1>
      <div className='w-full p-6 py-7  rounded  md:w-1/2 lg:w-1/3 mx-auto shadow-lg'>
        <form className=' flex flex-col' onSubmit={handleSubmit(onSubmit)}>
            <TextInput  {...register("name")} label="Name" placeholder='name' icon={<AiOutlineUser size="0.8rem"/>}/>
            <InputError message={errors.name?.message}/>
            <TextInput  {...register("email")} className='mt-3' label="Email" placeholder='email' icon={<MdAlternateEmail size="0.8rem"/>}/>
            ...
            <PasswordInput  {...register("password_confirmation")} className='mt-3' label="Confirm Password" placeholder='confirm password' icon={<Si1Password size="0.8rem"/>}/>
